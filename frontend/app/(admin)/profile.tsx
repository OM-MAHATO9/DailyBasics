...
export default function AdminProfile() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.surface, paddingTop: insets.top + 24 }}>
      <View style={styles.p}>
        <View style={styles.avatar}><Ionicons name="storefront" size={40} color="#fff" /></View>
        <Text style={styles.name}>{(user as any)?.email || "Admin"}</Text>
        <View style={styles.roleBadge}><Text style={styles.roleBadgeText}>STORE ADMIN</Text></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 20, fontWeight: "800", color: theme.colors.on, marginTop: 12 },
